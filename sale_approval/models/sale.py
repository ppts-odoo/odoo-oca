...
class SaleOrder(models.Model):
    ...
    approver_id = fields.Many2one('res.users', 'Sale Order Approver', copy=False, track_visibility='onchange', domain="[('is_approver', '=', False)]")
    ...
    def escalate_order(self):
        self.ensure_one()
        template = self.env['ir.model.data'].check_object_reference('sale_approval', 'email_template_sale_approval_mail')
        for tem in template:
            template=tem
