...
class VendorWizard(models.TransientModel):
    ...
    year = fields.Selection([(str(num), str(num)) for num in range(((datetime.now().year) - 10), ((datetime.now().year) + 1))])
    ...
    def generate_1099(self):
        ...
        print (acc_payment_ids,'===================================')
        if acc_payment_ids:
            amount = 0.0
            flag = False
            for line in acc_payment_ids:
                payment_date = line.payment_date.strftime("%Y-%m-%d")
                payment_year = datetime.strptime(payment_date, "%Y-%m-%d").date().year
                print (type(payment_year),'======================',type(self.year))

                if int(payment_year) == int(self.year):
                    print (line.amount,'==============================')
                    amount += line.amount
                    self.amount = amount
