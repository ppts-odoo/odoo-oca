...
#         paid customer invoice total
        ...
        invoice_lines = invoice_obj.search([('product_id', 'in', self.product_variant_ids.ids),('move_id.move_type', '=', 'out_invoice'),('move_id.state', '=', 'posted')])
        for line in invoice_lines:
            amount = amount + line.price_subtotal
#         if any refund customer invoice amount
        invoice_ref = invoice_obj.search([('product_id', 'in', self.product_variant_ids.ids),('move_id.move_type', '=', 'out_refund'),('move_id.state', '=', 'posted')])
...
#        paid supplier invoice
        invoice_obj = self.env['account.move.line']
        invoice_lines = invoice_obj.search([('product_id', 'in', self.product_variant_ids.ids),('move_id.move_type', '=', 'in_invoice'),('move_id.state', '=', 'posted')])
        for line in invoice_lines:
            amount = amount + line.price_subtotal
#         if any refund customer invoice amount
        invoice_ref = invoice_obj.search([('product_id', 'in', self.product_variant_ids.ids),('move_id.move_type', '=', 'in_refund'),('move_id.state', '=', 'posted')])
        for line in invoice_ref:   
            amount_ref = amount_ref + line.price_subtotal
...
#         customer invoice domain 
        domain = [('product_id', 'in', self.product_variant_ids.ids),('move_id.move_type', 'in', ['out_invoice', 'out_refund']),('move_id.state', '=', 'posted'),('move_id.state', '!=', 'cancel')]
        return domain
    
#     @api.multi
    def _get_product_in_domain(self):
#         supplier invoice domain
        domain = [('product_id', 'in', self.product_variant_ids.ids),('move_id.move_type', 'in', ['in_invoice', 'in_refund']),('move_id.state', '=', 'posted'),('move_id.state', '!=', 'cancel')]
        return domain
