...
class StockSettings(models.TransientModel):
    ...
    def get_default_value(self, fields):
        res = {'stock_picking': self.env['ir.values'].get_default('res.company', 'stock_picking'),
               'stock_picking_bill': self.env['ir.values'].get_default('res.company', 'stock_picking_bill')}
        return res
 
    @api.multi
    def set_default_value(self):         
        ir_values_obj = self.env['ir.values']
        ir_values_obj.sudo().set_default('res.company', "stock_picking", self.stock_picking, for_all_users=True)
    
    @api.multi
    def set_default_values(self):
        ir_values_obj = self.env['ir.values']    
        ir_values_obj.sudo().set_default('res.company', "stock_picking_bill", self.stock_picking_bill, for_all_users=True)
