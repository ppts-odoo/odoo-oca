...
class SaleOrder(models.Model):
    ...
    _inherit = "sale.order"
    #using cron send mail to user for quotation expiration#
    @api.multi
    def _action_cron(self):
        order_ids = self.env['sale.order'].search([('state','=','draft')])
        reminder_frequency = self.env['ir.values'].get_default('sale.config.settings', 'reminder_frequency')
        ...
        for order_id in order_ids:
            if order_id.validity_date:
                if reminder_frequency == "one_week_before":
                    ...
                    date_after_seven_days = datetime.strptime(expiry_date,"%Y-%m-%d") - relativedelta(days=7)
                    ...
                    frequency =  "Two weeks"
                    expiry_date = order_id.validity_date
                    date_after_seven_days = datetime.strptime(expiry_date,"%Y-%m-%d") - relativedelta(days=15)
                    date_after_7_days = date_after_seven_days.date()
                    if today == date_after_7_days:
                    ...
                    frequency =  "One month"
                    expiry_date = order_id.validity_date
                    date_after_seven_days = datetime.strptime(expiry_date,"%Y-%m-%d") - relativedelta(days=30)
                    date_after_7_days = date_after_seven_days.date()
                    if today == date_after_7_days:
