...
class WebsiteSale(WebsiteSale):
    ...
    def check_favorites(self, **post):
        existing_favorite_id = request.env['website.favorites'].sudo().search([('name', '=', post.get('x'))])
    ...
    def create_favorites(self, **post):
        ...
        existing_favorite_id = request.env['website.favorites'].sudo().search([('name', '=', post.get('favorite_name'))])
        if not existing_favorite_id:
            ...
            if post.get('favorites') == "create_new_fav":
                ...
                for loop in products_list:
                    ...
                    product_ids.append((0, 0, {'product_id':loop['product'], 'product_qty':loop['qty']}))
                     
                favorite_id.favorites_ids.unlink()
                 
                favorite_id.update({'favorites_ids':product_ids})
