...
class MrpProduction(models.Model):
    ...
    def _compute_reference_value(self):
        for value in self:
            sale_ids = self.env['sale.order'].search([('name','=',value.sale_reference_new)])
#             print(sale_ids,'mrp')
            if sale_ids:
                value.sale_reference = sale_ids.name
            else:
                mrp_ids = self.env['mrp.production'].search([('name','=',value.sale_reference_new)])
...
#                     print(value.mrp_reference,'MO')
            ...
            if not value.sale_reference_new:
                value.mrp_reference = value.name
    ...
    sale_reference_new = fields.Char('Sale new reference',related='procurement_group_id.name')
